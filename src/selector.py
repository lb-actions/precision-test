...
# Can be overridden via REPO_NAME environment variable
REPO_NAME = os.environ.get("REPO_NAME", "vllm_ascend")
...
COVERAGE_DENSITY_THRESHOLD = float(os.environ.get("COVERAGE_DENSITY_THRESHOLD", "0.0"))
...
def _get_test_files_from_pr_diff(diff_file: str) -> list[str]:
    ...
    # Test files must be in tests/e2e/pull_request/ or tests/ut/ directory
...
class CoverageSelector:
    ...
    def normalize_test_name(test_name: str) -> str:
        ...
        - tests__e2e__... -> tests/e2e/... (file-level) -> tests/e2e/....py
        - tests__e2e__...--test_foo -> tests/e2e/...::test_foo (function-level)
    ...
    def build_test_case_map(self) -> dict:
        ...
        for i, test_case in enumerate(test_cases):
            ...
            print(
                f"    -> {len(file_lines_map)} files, "
                f"{sum(len(v) for v in file_lines_map.values())} lines"
            )
        ...
        for raw_line in diff_output.split("\n"):
            ...
            # hunk header: @@ -old_start,old_count +new_start,new_count @@
                ...
                if match:
                    ...
                    # Collect all lines in hunk, check if there are new lines
                    hunk_lines = []
                    split_lines = diff_output.split("\n")
                    start_idx = split_lines.index(line) + 1
                    for hunk_line in split_lines[start_idx:]:
                        if (
                    ...
                    # If no new lines starting with +, changes only include deletions
                    # shrink range by one line
                    has_addition = any(
...
class TestSelector:
    ...
    def select_tests(
        self,
        config: SelectTestsConfig,
    ) -> tuple[list[tuple[str, dict[str, set[int]], int]], str]:
        """
        Select affected test cases based on changed files, supports 3 independent
        ...
        Each granularity cascades: only when current granularity finds no tests,
        try the next.

        Args:
            config: Configuration for test selection

        Returns:
            (selected_tests, expand_reason)
            - selected_tests: [(test_case_name, {filepath: {covered_lines}},
                total_affected_lines), ...]
            - expand_reason: expansion reason
                ('' means no expansion, 'line'/'function'/'file' indicates the
                granularity used)
        """
        selected = []
        expand_reason = ""
        ...
        normalized_changed = {}
        for f, lines in config.changed_files_with_lines.items():
        ...
        # Line-level matching + Function-level matching (parallel execution)
        ...
        if config.enable_line_match:
            for test_case, data in self.test_case_map.items():
                covered_files = data["files"]  # {filepath: {lineno, ...}}

                # Line-level matching: calculate which changed lines are covered
                affected_detail = {}  # {filepath: set of covered changed lines}
                all_intersected_lines = set()  # union of intersections across files
                ...
                # Calculate overall coverage density
                overall_density = (
                ...
                if (
                    ...
                    and len(all_intersected_lines) >= config.min_affected_lines
                ):
                    line_results.append(
            ...
            if line_results and config.enable_dedup:
                claimed_lines = set()
                deduplicated = []
        ...
        if config.enable_function_match and config.source_dir:
            # Collect functions that changed lines belong to
            changed_functions = {}  # {filepath: {func_name: Set[linenos]}}

            for changed_file, changed_lines in normalized_changed.items():
                possible_paths = [
                    Path(config.source_dir) / changed_file,
                    Path(config.source_dir) / REPO_NAME / changed_file,
                ...
                line_to_function = FunctionParser.get_lines_functions(
                    source_file,
                    changed_lines,
                    skip_imports=config.enable_skip_imports,
                )
            ...
            if changed_functions:
                    ...
                    for changed_file, func_to_lines in changed_functions.items():
                        ...
                        for func_name in func_to_lines:
                            ...
                            for func_start, func_end in func_ranges[func_name]:
                                func_all_lines.update(
                                    range(func_start, func_end + 1)
                                )
                            ...
                            if covered_in_func:
                                # Get intersection of test covered lines and changed lines
                ...
                # Select tests that cover other lines of changed functions
                for changed_file, func_to_lines in changed_functions.items():
                    for func_name in func_to_lines:
                        if func_name in func_to_tests:
                            for test_case, covered_in_func, covered_changed_lines in (
                            func_to_tests[func_name]
                        ):
                                existing = [s[0] for s in func_results]
                                if test_case not in existing and covered_in_func:
                                    # Display changed lines coverage if available
                                    display_lines = (
                                        covered_changed_lines
                                    ...
                                    print(
                                        f"  [Function match] {test_case} covers "
                                        f"function '{func_name}' in {changed_file}"
                                    )
        ...
        if line_results or func_results:
            ...
            if selected:
                print(
                    f"  Line match: {len(line_results)} tests, "
                    f"Function match: {len(func_results)} tests, "
                    ...
                    if func_name in func_to_tests:
                        for test_case, covered_changed_lines in func_to_tests[func_name]:
                            ...
                            if test_case not in existing and covered_changed_lines:
                                # Display actual changed line coverage
                                display_lines = (
                                    covered_changed_lines
                                    if covered_changed_lines
                                    else set()
        ...
        if not selected and config.enable_file_match:
            ...
            for test_case, data in self.test_case_map.items():
                ...
                for changed_file in normalized_changed:
                    if changed_file in covered_files:
                        ...
                        if covered_lines:
                            selected.append(
                                (test_case, {changed_file: covered_lines}, len(covered_lines))
                            )
                            break
    ...
    def print_selection(
        ...
        if expand_reason and expand_reason in gran_names:
            ...
            print(
                f"Selected: {len(selected)} test cases "
                f"(min affected: {min_affected_lines} lines)"
            )
        print("=" * 70)
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(only effective for function-level matching, default off)"
        ),
    )
    ...
    if has_csrc_changes:
        ...
        if changed_files_with_lines:
            ...
            config = SelectTestsConfig(
                changed_files_with_lines=changed_files_with_lines,
            ...
            selected, expand_reason = test_selector.select_tests(config)
